package com.angema.hours.app.core;

public class Constant {

    public static final int MIN_CHARACTER_MAIL = 8;
    public static final int MIN_CHARACTER_PASSWORD = 6;
    public static final int MIN_CHARACTER_NAME = 4;
    public static final int MIN_CHARACTER_SURNAME = 4;
    public static final int MIN_CHARACTER_COMPANY = 2;
    public static final int MIN_CHARACTER_PHONE = 8;
    public static final int MIN_CHARACTER_CUIT = 4;
    public static final int MIN_CHARACTER_DIRECTION = 6;
    public static final int MIN_VALUE_HOURS = 1;

    public static final int MAX_CHARACTER_MAIL = 30;
    public static final int MAX_CHARACTER_PASSWORD = 15;
    public static final int MAX_CHARACTER_NAME = 30;
    public static final int MAX_CHARACTER_SURNAME = 30;
    public static final int MAX_CHARACTER_COMPANY = 20;
    public static final int MAX_CHARACTER_PHONE = 15;
    public static final int MAX_CHARACTER_CUIT = 15;
    public static final int MAX_CHARACTER_DIRECTION = 50;
    public static final int MAX_VALUE_HOURS = 24;
}
